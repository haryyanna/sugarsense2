...
const callGemini = async ({ model, contents, maxOutputTokens = 800 }) => {
  if (!GEMINI_API_KEY) throw new Error('Gemini API belum dikonfigurasi. Pastikan GEMINI_API_KEY tersedia pada GitHub Actions.');
  const response = await fetch(`${GEMINI_API_BASE}/${model}:generateContent?key=${encodeURIComponent(GEMINI_API_KEY)}`, {
    method: 'POST',
...
const analyzeImageWithGemini = async (imageB64) => {
  ...
  for (const model of GEMINI_MODELS) {
    try {
      ...
      if (normalizedAnswer.includes('not_a_drink') || normalizedAnswer.includes('not a drink')) throw new Error('Objek pada foto bukan minuman. Silakan arahkan kamera ke minuman atau kemasannya.');
...
export const chatCompletion = async ({ messages, max_tokens = 800 }) => {
  // GitHub Pages cannot execute /api/*.js, so use Gemini directly in the browser.
  if (GEMINI_API_KEY) {
    let lastError = null;
    for (const model of GEMINI_MODELS) {
      try {
        const systemInstruction = messages.find((message) => message.role === 'system')?.content || 'Kamu adalah SugarSense AI, asisten gizi minuman. Jawab sesuai konteks percakapan dan jangan mengarang data nutrisi.';
        ...
        const response = await callGemini({
          ...
          contents: [
            ...
            { role: 'model', parts: [{ text: 'Baik, saya akan mengikuti instruksi dan mempertahankan konteks percakapan.' }] },
            ...contents
          ],
          maxOutputTokens: Math.max(max_tokens, 800)
        });
        return { ok: true, content: response.text, model: response.model };
      } catch (error) { lastError = error; }
    }
    throw lastError || new Error('Gemini chat gagal.');
  }
  if (API_BASE) return request('/api/chat/completions', { method: 'POST', body: { messages, max_tokens } });
  throw new Error('Gemini API belum tersedia.');
};
...
export const analyzeDrinkImage = async ({ imageB64, searchHint, drinkKey }) => {
  ...
  if (selectedDrink) return { ok: true, result: selectedDrink, source: 'verified-database' };
  ...
  if (hintedDrink) return { ok: true, result: hintedDrink, source: 'verified-database' };
  throw new Error('Gemini API belum tersedia. Pilih jenis minuman pada bagian Bantuan Identifikasi lalu tekan Analisis Gizi.');
};

export const searchDrinks = async (query) => request(`/api/fatsecret/search?q=${encodeURIComponent(query)}`);
